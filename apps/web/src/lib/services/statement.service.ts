...
export class StatementService {
  async apartmentStatement(apartmentId: string) {
    ...
    const rents = (await sheets.getAll("rents")).filter(
      (r) => r.apartment_id === apartmentId
    );

    const bills = (await sheets.getAll("bills")).filter(
      (b) => b.apartment_id === apartmentId
    );

    const allocations = await sheets.getAll("allocations");

    const rentLedger = resolveAllocations(rents, allocations, "rent_id", "rent_id");
    const billLedger = resolveAllocations(bills, allocations, "bill_id", "bill_id");
  ...
  async tenantStatement(tenantId: string) {
    ...
    const occIds = occupancies.map((o) => String(o.occupancy_id));

    const rents = (await sheets.getAll("rents")).filter((r) =>
      occIds.includes(String(r.occupancy_id))
    );

    const bills = (await sheets.getAll("bills")).filter((b) =>
      occIds.includes(String(b.occupancy_id))
    );
